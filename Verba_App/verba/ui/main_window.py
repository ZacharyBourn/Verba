...
class MainWindow:
    ...
    def show_stats_window(self):
        ...
        stats_grid = tk.Frame(card, bg=self.panel_color)
    ...
    def show_vocab_window(self):
        ...
        list_frame = tk.Frame(window, bg=self.bg_color)
        ...
        for entry in entries:
            ...
            if chapter_title:
                display = f"{word}  |  {chapter_title}  |  {preview}"
            else:
                display = f"{word}  |  {preview}"
        ...
        tk.Button(
            ...
            command=lambda: self.delete_vocab_entry(window, listbox)
    ...
    def delete_vocab_entry(self, window, listbox):
        selection = listbox.curselection()
        if not selection:
        ...
        word = entries[index].get("word", "").strip() or "this entry"
        confirm = messagebox.askyesno("Delete Vocab Entry", f"Delete '{word}' from vocab?")
        if not confirm:
            return

        self.vocab_manager.remove_entry(index)
        window.destroy()
        self.show_vocab_window()
